...
>
> **Note (added 2026-06-10 after second/third PR push):** Two further fixes turned out to be required after CI runs surfaced them. (1) `-Drelease` deactivates the `default` profile in each `vaadin-X-flow-parent/pom.xml` that declares the IT submodule, so `mvn verify -am -pl <IT module path>` fails with "Could not find the selected project in the reactor". Solution: drop `-Drelease` from the IT shard mvn invocation. (2) The outer workspace's `package.json` originally listed only `web-components` and `web-components/packages/*` as members, but `web-components/package.json` declares nested workspaces (`test/*`, `packages/*`, `api-docs`, `dev`). With those nested members invisible to the outer workspace, npm install hoisted some packages to the workspace-root `node_modules/` and others to `web-components/node_modules/`, which broke Vite's resolution inside IT modules. Solution: flatten the nested workspaces into the root `package.json` (`web-components/test/*`, `web-components/dev`, `web-components/api-docs`), delete `package-lock.json`, and let `npm install` regenerate with a single hoist tree. Both fixes are folded into the relevant tasks below.
>
> **Note (added 2026-06-10 after fourth PR push):** Grid IT (shard 1) still failed with `Could not execute build-frontend goal: Unable to locate file @vaadin/grid/src/vaadin-grid-column-group.js`. Diagnosis: `TemplatedColumnsPage extends LitTemplate` declares `@JsModule("@vaadin/grid/src/vaadin-grid-column-group.js")`. Flow's `TaskCopyTemplateFiles` physically locates each `@JsModule` path via `FrontendUtils.resolveFrontendPath(<IT-module>, path, <IT>/frontend)` — which checks `<IT>/node_modules/`, `<IT>/frontend/`, and `<IT>/frontend/generated/jar-resources/` only, never walking up to the workspace root. Under hoisting the file sits at `<workspace-root>/node_modules/@vaadin/grid → web-components/packages/grid`, so the lookup misses. There is no built-in skip for `TaskCopyTemplateFiles` (the only Flow-plugin-level skip, `vaadin.skip`, kills the whole plugin and breaks IT bundling). Solution: `scripts/sync-flow-overlays.sh` creates one targeted, cycle-safe symlink at `flow-components/vaadin-grid-flow-parent/vaadin-grid-flow-integration-tests/node_modules/@vaadin/grid → ../../../../../web-components/packages/grid`. Locally verified: `Unable to locate file` is gone with the symlink in place. If another IT module ever trips the same failure, add a similar narrow entry — no blanket per-`@vaadin/*` symlinks.
...
## Task 1b: Flatten `web-components`' nested workspaces into the root

**Files:**
- Modify: `package.json` (workspaces array)
- Modify: `package-lock.json` (regenerated by npm)

`web-components/package.json` declares nested workspaces (`test/*`, `packages/*`, `api-docs`, `dev`). The outer workspace at the repo root only listed `web-components` and `web-components/packages/*`, leaving the nested `test/*`, `dev`, and `api-docs` invisible to the outer hoist algorithm. With those members invisible, npm install put some packages at the workspace-root `node_modules/` and others under `web-components/node_modules/`. Vite running inside an IT module then saw split resolution — `@vaadin/*` resolved correctly at workspace root, but some transitive deps couldn't be located. Adding the nested entries to the outer workspaces array makes the entire dep graph hoist into a single tree.

### Steps

- [ ] **Step 1b.1: Add the nested workspaces to the root `package.json`.**

Open `package.json`. The `workspaces` array currently looks like:
...
Insert three additional entries after `web-components/packages/*`:
...
These mirror what `web-components/package.json` declares under its own `workspaces` array.

- [ ] **Step 1b.2: Delete `package-lock.json` and regenerate.**
...
The freshly regenerated `package-lock.json` should now consolidate every workspace member's deps into a single hoist tree rooted at `<workspace-root>/node_modules/`. Verify:

```bash
ls web-components/node_modules 2>&1 | head -3
```

Expected: `No such file or directory` (everything hoisted to root). If `web-components/node_modules/` exists with content, the workspaces array didn't take effect — re-check `package.json`.

- [ ] **Step 1b.3: Verify an IT module's frontend graph resolves.**

```bash
cd flow-components && mvn flow:prepare-frontend flow:build-frontend \
  -pl vaadin-button-flow-parent/vaadin-button-flow-integration-tests \
  -am -Drun-it -Dvaadin.productionMode -Dvaadin.force.production.build=true \
  -DskipUnitTests -DskipTests -DskipJetty -B -ntp
```

Expected: Flow's `build-frontend` goal completes (no "Failed to find imports in the `node_modules` tree" errors). Vite's bundling may then surface unrelated TypeScript or platform-specific issues — those are out of scope; only the pre-bundle import scan needs to pass.
...
git commit -m "$(cat <<'EOF'
fix(workspace): include web-components nested workspaces at the root

web-components/package.json declares nested workspaces (test/*, packages/*,
api-docs, dev) but only `web-components` and `web-components/packages/*`
were exposed at the outer workspace level. As a result, npm install
hoisted some deps to the workspace-root node_modules/ and others to
web-components/node_modules/, which broke Vite's resolution inside IT
modules at production-build time.

Flatten all nested members into the root workspaces array so the entire
dep graph hoists into a single tree. Regenerate package-lock.json to
match.
...
- [ ] **Step 2.7: Add `flow-components/**/node_modules` to every cache path list.**

The install cache currently lists `node_modules`, `web-components/node_modules`, `web-components/.yarn`, and a few other paths. Add `flow-components/**/node_modules` to all five places this list appears (one save in `install`, four restores in `install`, `unit`, `wtr`, `its`). After the workspace-config fix in Task 1b, most deps hoist to the workspace-root `node_modules/`, but a small `flow-components/node_modules/@types/*` directory remains for non-hoistable type packages. Caching it keeps the IT shards from re-resolving anything that ended up there.

`web-components/node_modules` and `web-components/.yarn` no longer get created after the workspace-config fix; the entries are dead but harmless (the cache action silently skips missing paths). Leave them in place — removing them is unrelated cleanup.

- [ ] **Step 2.8: Validate the workflow YAML parses.**
...
- [ ] **Step 2.9: Sanity-check the final job set.**
...
- [ ] **Step 2.10: Sanity-check the `its` job's needs and matrix wiring.**
...
- [ ] **Step 2.11: Make `:web-components:install` a true no-op.**

In `gradle/web-components.gradle.kts`, drop the `dependsOn(":npmInstall")` line from the `install` task so it becomes a pure no-op. The root `:install` task already triggers `:npmInstall` directly (see `build.gradle.kts`), so the workspace install still happens exactly once; this change just stops `./gradlew :web-components:install` (invoked on its own) from re-pulling the workspace install.
...
```bash
./gradlew :web-components:install --dry-run
```

Expected output ends with `:web-components:install SKIPPED` and **no** `:npmInstall` line — confirming the dependency is truly gone.

- [ ] **Step 2.12: Commit.**

```bash
git add .github/workflows/validation.yml gradle/web-components.gradle.kts
git commit -m "$(cat <<'EOF'
...
- **Per-module frontend build failing with "Failed to find the following imports in the `node_modules` tree"**: workspace hoisting is split across `node_modules/` and `web-components/node_modules/`. Re-check the root `package.json` workspaces array — it must include `web-components/test/*`, `web-components/dev`, and `web-components/api-docs` per Task 1b. After fixing, delete `package-lock.json` and let `npm install` regenerate.
...
- **Spec §Overview & Goals** — Task 2 deletes `package-war` and the global merge step. Task 1 implements LPT bin-packing. Task 1b fixes the npm workspace's nested-membership gap. Task 3 documents the evaluation procedure (PR runs vs. recent main runs).
- **Spec §Evaluation Procedure** — Reproduced in Task 3's PR description template.
- **Spec §Changes to `.github/workflows/validation.yml`** — Task 2 steps 2.1–2.7 each map to a bullet in this section. Step 2.4 covers the three sub-edits to `its` (drop `package-war` from `needs`, drop WAR-cache restore, replace run command **without `-Drelease`**). Step 2.5 covers the upload-path changes. Step 2.7 adds `flow-components/**/node_modules` to every cache path list. Step 2.11 makes `:web-components:install` a true no-op in Gradle.
- **Spec §Changes to `scripts/compute-it-matrix.sh`** — Task 1 step 1.3 contains the rewritten script content.
- **Spec §Notable differences "Workspace must include `web-components`' nested workspaces"** — Task 1b implements that fix.
...
- **Spec §Verification step 2** — Task 3 step 3.3 covers the workflow-behaviour check (modular run green; troubleshooting list calls out the `-Drelease` reactor-discovery failure mode and the workspace-hoist split failure mode explicitly).
